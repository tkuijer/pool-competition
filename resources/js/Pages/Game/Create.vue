<template>
    ...
    <div v-if="activeStep === 3">
        <h2 class="font-bold pb-4 text-xl">
            Met welke keu speelde {{ winner.name }}?
        </h2>
        <ul class="grid grid-cols-2 gap-x-5">
            <li v-for="cue in cues">
                <button
                    class="block text-lg py-3 rounded-lg border border-gray-200 w-full hover:bg-blue-50 flex justify-center"
                    @click="form.winner_cue = cue; activeStep++"
                >
                    {{ cue }}
                </button>
            </li>
    ...
    <div v-if="activeStep === 5">
        <h2 class="font-bold pb-4 text-xl">
            Met welke keu speelde {{ opponent.name }}?
        </h2>
        <ul class="grid grid-cols-2 gap-x-5">
            <li v-for="cue in cues">
                <button
                    class="block text-lg py-3 rounded-lg border border-gray-200 w-full hover:bg-blue-50 flex justify-center"
                    @click="form.opponent_cue = cue; activeStep++"
                >
                    {{ cue }}
                </button>
            </li>
    ...
    <div v-if="activeStep === 7">
        ...
        <span class="bold">Winnaar: </span>{{ winner.name }} ({{ form.winner_cue }})<br/>
        Speelde: {{ form.winner_color }} <br/>
        Tegen: {{ opponent.name }} ({{ form.opponent_cue }})<br/>
