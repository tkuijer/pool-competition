<template>
    ...
    <div class="flex justify-center gap-4 h-72 mb-5">
        ...
        <pie-chart :data="stats.winning_cues" :options="{
            plugins: {
                legend: {
                    position: 'bottom',
                }
    ...
    <div v-for="game in games.data" class="border-b mb-4">
        ...
        <div class="text-lg">
            <div class="underline">
                Winnaar: <span class="italic">{{ game.winner.name }} ({{ game.winner.pivot.cue_number }})</span>
            </div>
            Playing the {{ game.winner.pivot.color }} ball. Won by: {{ game.win_method }}
        </div>
        <div>
            Opponent: <span class="italic">{{ game.loser.name }} ({{ game.loser.pivot.cue_number }})</span>
        </div>
    </div>
